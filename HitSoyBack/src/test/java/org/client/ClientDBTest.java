...
public class ClientDBTest {
	...
	public void testClientBD_createAndOpen() throws Exception{
		...
		assertNotNull("clientDB est crée",clientDB.ClientDB_create("../clientdb_unittest.db"));
		assertEquals("Est ce que clientDB est vide",0,clientDB.getRecordCount());
		clientDB.ClientDB_close();
		
		assertNotNull("clientDB est ouvert",clientDB.ClientDB_open("../clientdb_unittest.db"));
		assertEquals("Est ce que clientDB est vide",0,clientDB.getRecordCount());
		assertEquals("Est ce que clientDB est vide",0,clientDB.ClientDB_getRecordCount(clientDB));
		clientDB.ClientDB_close();
		
		clientDB.ClientDB_remove("../clientdb-unittest-doesnotexist.db");
		assertEquals("Est ce que clientDB est vide",null,clientDB.ClientDB_open("../clientdb-unittest-doesnotexist.db"));
	}
	...
	public void testClientBD_readAndWrite() throws Exception{
		...
		for(int i=0;i<100;i++){
			...
			assertEquals("Est ce que clientDB est "+i+1,i+1,clientDB.getRecordCount());
		}
		clientDB.ClientDB_close();
	...
	private void testClientDB_append() {
		...
		assertEquals("Est ce que clientDB record est 1",1,clientDB.getRecordCount());
		clientDB.ClientDB_appendRecord(clientRecord);
		assertEquals("Est ce que clientDB record est 2",2,clientDB.getRecordCount());
		clientDB.ClientDB_close();
		...
		clientDB.ClientDB_remove("../clientdb-unittest-doesnotexist.db");
		assertNotNull("clientDB est cr�� ou ouvert",clientDB.ClientDB_openOrCreate("../clientdb-unittest-doesnotexist.db"));
		assertEquals("Est ce que clientDB est vide",0,clientDB.getRecordCount());
		clientDB.ClientDB_close();
